...
public:
	...
	#ifdef UNICODE
		wstring tempWstring(name.begin(), name.end());
		LPCWSTR tempLPCWSTR = tempWstring.c_str();

		PlaySound(tempLPCWSTR, NULL, SND_LOOP | SND_ASYNC);
	#else
		PlaySound(name.c_str(), NULL, SND_LOOP | SND_ASYNC);
	#endif
